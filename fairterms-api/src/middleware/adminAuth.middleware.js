 ...
 *  1. Firebase custom claim: { role: 'admin' } OR { admin: true }
 *  2. ADMIN_UIDS env var (comma-separated UIDs)
 */
async function adminAuthMiddleware(req, res, next) {
  ...
  if (admin.apps.length > 0) {
    try {
      ...
      if (userRecord.customClaims?.role === 'admin' || userRecord.customClaims?.admin === true) {
        return next();
      }
    } catch (err) {
